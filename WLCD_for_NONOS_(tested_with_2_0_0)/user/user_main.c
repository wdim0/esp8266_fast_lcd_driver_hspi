...
void ICACHE_FLASH_ATTR user_init(void){
	...
	uint8_t* ImgData = os_malloc(8000); //arrays allocated by malloc() are also always 4-bytes aligned (at least on ESP8266 SDK). Make sure that the buffer is big enough
	...
	while(1){
		...
		//
		ets_sprintf(Str, "WLCD %s", wlcd_get_version());
		...
		S.MaxH = WLCD_HEIGHT-20;
		S.WrapStyle = WLCD_WRAP_WORDS;
		ets_sprintf(Str, "1000 images (50x50 pixels, uncompressed R5G6B5) took %d ms", t/1000);
		wlcd_text_draw(Str, &S);
		OS_DELAY_MS(5000);
		//
