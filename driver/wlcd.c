...
#include "wlcd_defs.h" //must be included before esp_common.h to win (in esp_common.h, there's also #include for standard spi_register.h which is incomplete)
...
#define WLCD_VERSION			"v1.0" //03/2016
...
//#define DELAY					vTaskDelay(1) //debug - oscilloscope - for really slow bit-banging
 ...
 * Please use "__attribute__ ((aligned(4)))" prefix then declaring arrays passed as Buf
 * or use malloc(...) - at least on ESP8266 the malloc(...) also allocates 4-byte aligned arrays.
 * Also, theres definition WLCD_ALIGN4_ATTR for you
 * uint8_t WLCD_ALIGN4_ATTR MyBuf[10*4];
 * uint32_t WLCD_ALIGN4_ATTR MyBuf[10];
 * uint8_t* MyBuf = malloc(10*4);
 ...
 * uint32_t WLCD_ALIGN4_ATTR MyBuf[(MYBUF_BYTES/4)+(MYBUF_BYTES % 4)?1:0]; //this will allocate 65 x uint32_t = 260 bytes
 *
 ...
 */
	...
	uint32_t* HSPIBuf;
	uint8_t Dup16sDone;
	uint32_t WLCD_ALIGN4_ATTR TwoRevDup16s;
	//
 ...
 */
...
#ifdef WLCD_USE_HSPI
	DBG_WLCD("initializing HSPI/GPIOs for LCD interface (HW mode, HSPI CLK = %d kHz)\n", ((uint32_t)system_get_cpu_freq()*1000) / WLCD_SPI_CLK_PREDIV / WLCD_SPI_CLK_CNTDIV);
	//
	...
	//
#else
	DBG_WLCD("initializing GPIOs for LCD interface (SW mode)\n");
	//
	...
	DBG_WLCD("initializing display controller ILI9341\n");
	//
...
//Sets LCD controller drawing area - this area will be filled by incoming pixel data (for fill / image / line / ...).
	uint8_t WLCD_ALIGN4_ATTR Buf[4];
	//
	uint8_t Idx = 0;
